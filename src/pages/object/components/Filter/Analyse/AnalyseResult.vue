<template>
  ...
  <v-row>
    <v-col v-for="item in show_list" :key="item" cols="4" sm="12" lg="4" xl="3" xxl="2">
      <h-card :title="item.title" class="transition-all duration-300 ease-in-out hover:shadow-lg">
        <template #btns>
          ...
          <v-btn icon="mdi-window-maximize" density="compact" variant="text" class="text-xs"
            @click="maximizeItem(item)" />
